...
export async function getTmClient(): Promise<
...
> {
  ...
  const rpc =
    "https://and76cjzpa.execute-api.us-east-2.amazonaws.com/wormchain";
...
export async function queryWormchain(token: string, chainId: ChainId) {
  ...
  const res = await client.searchTx([
    ...
    {
      key: "wasm.token_address",
      value: `ExternalTokenId { bytes: [${bytes.join(", ")}] }`,
    },
  ]);
  return res;
}
