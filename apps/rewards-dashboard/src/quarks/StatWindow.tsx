...
export const StatWindow = (props:StatWindowProps)=>{
  ...
  return (<div className={`
    ...
    <div className="flex flex-col pr-8 gap-2
      ">
      <div className="text-white text-start whitespace-pre text-sm">{header}</div>
      <div className="flex flex-row items-end grow">
        {value ? <div className="text-white whitespace-pre text-start font-bold text-2xl">{
          value
              }</div> : <div className="
                w-full
                animate-pulse
        ...
        "/>}
        {(value && unit) ? <div className="pl-2 text-white text-sm whitespace-pre text-start">
          {unit}
        </div> : ""}
      </div>
    </div>
  ...
  return (<div className={`
    ...
    <div className="flex flex-col pr-8 w-full
      ...
      <div className="flex flex-row items-end">
        ...
        "/>}
        {(value && unit) ? <div className="pl-2 text-white text-sm whitespace-pre text-start -translate-y-0.5">
          {unit}
        </div> : ""}
      </div>
    </div>
    <InfoHover><div
      className="prose text-[10px] text-white"
    >{infoElement ? infoElement : "Tooltip"}</div></InfoHover>
  </div>)
}
