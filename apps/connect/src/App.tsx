...
import { ExtendedTransferDetails, LiquidMarketsWarningContextType, ValidateTransferHandler, ValidateTransferResult, WarningLiquidMarkets, validateTransfer } from "./providers/liquid-markets";
...
export default function Root() {
  ...
  const [warning, setWarning] = useState<LiquidMarketsWarningContextType>({ warningType: WarningLiquidMarkets.NO_WARNING, corridor: undefined });
  const [open, setOpen] = useState(false);

  const ref = useRef<((e: ValidateTransferResult) => void) | null>(null);
  const actionResult = useCallback((resolve: (e: ValidateTransferResult) => void) => {
    ref.current = resolve;
  }, [ref]);

  const validate: ValidateTransferHandler = useCallback(async (e: ExtendedTransferDetails): Promise<ValidateTransferResult> => {
  ...
  return (
    <>
      ...
      <ModalWarning isOpen={open} warningType={warning.warningType} corridor={warning.corridor} onClose={onClose} onContinue={onContinue} />
