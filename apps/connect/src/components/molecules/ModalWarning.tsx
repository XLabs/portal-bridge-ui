...
const TransferLimitedWarning = ({
  ...
  const explorerURL = !corridor?.toChain
    ? ""
    : getExplorerURL(corridor?.toChain);
  const chainName = !corridor?.toChain
    ? ""
    : toChainNameIcon(corridor?.toChain);
  if (
    warningType === WarningLiquidMarkets.MANUALLY_WARNED_CORRIDOR ||
    warningType === WarningLiquidMarkets.RARE_CORRIDOR
  ) {
    ...
    message = (
      <>
        ...
        <br />
        <span>
          You will receive this token on {chainName}{" "}
          <span>
            <ChainIcon chain={corridor?.toChain} />
          </span>{" "}
          : <Link href={explorerURL}>{explorerURL}</Link>
        </span>
        <br />
        <br />
        Please confirm that you would like to transfer this token to {chainName}
        . If there isn't sufficient liquidity, you may need to transfer it back
        to {corridor?.fromChain}
      </>
    );
    footer = (
      <>
  ...
  if (warningType === WarningLiquidMarkets.NEW_CORRIDOR) {
    ...
    message = (
      <>
        This asset has never been bridged to {chainName}{" "}
        <ChainIcon chain={corridor?.toChain} /> on Wormhole before!
        <br />
        <br />
        If you think it should be supported, consider asking on{" "}
        <Link href="https://discord.com/invite/wormholecrypto">discord</Link>.
        <br />
        <br />
        Alternately, if you’re a developer, consider using the{" "}
        <Link href={`/advanced-tools/`}>Advanced Tool</Link>
      </>
    );
    footer = (
      <>
