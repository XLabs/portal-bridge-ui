import { CHAIN_ID_BSC, CHAIN_ID_ETH, CHAIN_ID_TERRA, CHAIN_ID_AURORA, ChainId, getForeignAssetTerra, hexToUint8Array, tryNativeToHexString } from "@certusone/wormhole-sdk";
...
const getTerraClassic = (originChain: ChainId, originAssetHex: string) => getForeignAssetTerra(
...
(async () => {
    const info =  await getTerraClassic(CHAIN_ID_ETH, '0xDeba6Ecc1865B217b1CaAbd16D7261d2a21b6046');
    console.log(info);
})()
...
const PandleMessage = 'Pandle transfers are limited to Ethereum to BSC and BSC to Ethereum.';
const AuroraMessage = 'As a precautionary measure, Wormhole Network and Portal have paused Aurora support temporarily.';
const TerraClassicMessage = 'Transfers of native tokens to/from Terra Classic have been temporarily paused.';
...
const TransferRules: Rule[] = [
    ...
    {
        id: 'aurora',
        predicate: ({ source, target }: PredicateArgs) => source === CHAIN_ID_AURORA || target === CHAIN_ID_AURORA,
        text: AuroraMessage,
        disableTransfer: true
    },
    {
        id: 'terra-classic-native',
        predicate: ({ source, token }: PredicateArgs) => source === CHAIN_ID_TERRA && terra.isNativeDenom(token),
