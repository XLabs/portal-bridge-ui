import { ChainId, ParsedVaa, parseTransferPayload, parseVaa, hexToUint8Array, tryHexToNativeString } from "@certusone/wormhole-sdk";
...
export function useRedeemControl(
        ...
        try {
            if (vaa) {
                return parseTransferPayload(
                    Buffer.from(new Uint8Array(vaa?.payload))
                )
            }
        } catch (e) {
            console.error(e);
    ...
    const asset = useMemo(() => tryHexToNativeString(payload?.originAddress || '', payload?.originChain as ChainId), [payload]);
    useEffect(() => {
