...
export function useTransferControl(
    ...
    useEffect(() => {
        const appliedRules = rules.filter(rule => rule.predicate({ source: sourceChain, target: targetChain, token: asset }));
        if (appliedRules.length > 0) {
            setWarnings(appliedRules);
            setIsTransferDisabled(appliedRules.some(rule => rule.disableTransfer));
            setIds(appliedRules.filter(rule => !!rule.id).map(rule => `${rule.id}`));
        } else {
            setWarnings([]);
            setIds([]);
